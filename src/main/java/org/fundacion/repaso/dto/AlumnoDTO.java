...
public class AlumnoDTO {
    ...
    private String dni;
    private List<NotaDTO> notas; // Aquí se agregan todas las notas
    ...
    public AlumnoDTO(Alumno al) {
        ...
        this.notas = toNotasDTO(al.getNotas()); // Metodo para convertir las notas en NotaDTO
    }

    // Metodo para convertir las notas de Alumno en NotaDTO
    public List<NotaDTO> toNotasDTO(List<Nota> notas) {
        List<NotaDTO> notasDTO = new ArrayList<>();
        for (Nota n : notas) {
            notasDTO.add(new NotaDTO(n)); // Convertir cada nota en NotaDTO
        }
        return notasDTO;
    }
}
